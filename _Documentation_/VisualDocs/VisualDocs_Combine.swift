...
extension UserDefaults {
    @objc var observedUserDefaultsVar: Float {
        get { return float(forKey: "myStoredUserDefaultsValue") }
        set { set(newValue, forKey: "myStoredUserDefaultsValue") }
    }
}
...
public class CombineTesting {
    ...
    public func testing_AnyPublisher() {
        func something() -> AnyPublisher<String, XError> {
    ...
    public func observe_UserDefaults() {
        ...
        UserDefaults.standard
            .publisher(for: \.observedUserDefaultsVar)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            ...
            UserDefaults.standard.observedUserDefaultsVar = Float(randomInteger)
        }
...
struct VisualDocs_Combine: View {
    let size_w: CGFloat = 50
    let size_h: CGFloat = 50
    var body: some View {
        ScrollView {
            Text("CombineTesting").font(.title)
            Group {
                Divider()
                Button(action: {
                    CombineTesting.shared.observe_UserDefaults()
                }) { Text("Observing UserDefaults") }
            }
            Group {
                Spacer()
                Button(action: {
                    CombineTesting.shared.testing_AnyPublisher()
                }) { Text("AnyPublisher") }
            }
            Group {
                Spacer()
            ...
            Group {
                ...
                }) { Text(".handleEvents(receiveSubscription(...") }
            }
        }
    }
}
