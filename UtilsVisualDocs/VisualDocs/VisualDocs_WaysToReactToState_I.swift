...
struct VisualDocs_WaysToReactToState_onEditingChanged: View {
    ...
    var body: some View {
        VStack {
            ...
            TextField("Write here:", text: $enteredTextValue, onEditingChanged: self.checkIfTextsMatch).applyDefaultLayout()
            Toggle(isOn: $textsMatch) { Text("Matching?") }.disabled(true).padding()
        }.padding()
    }
}

struct VisualDocs_WaysToReactToState_onEditingChanged_PreviewProvider: PreviewProvider {
    static var previews: some View {
        VisualDocs_WaysToReactToState_onEditingChanged()
    }
}
...
struct VisualDocs_WaysToReactToState_Binding: View {
    ...
    var body: some View {
        ...
        let theBody = VStack {
            ...
            TextField("Write here:", text: textValueBinding).applyDefaultLayout()
...
struct VisualDocs_WaysToReactToState_Binding_PreviewProvider: PreviewProvider {
    static var previews: some View {
        VisualDocs_WaysToReactToState_Binding()
    }
}
...
class ContentViewModel: ObservableObject {
    ...
    @Published var inputString: String = "" { didSet { checkIfTextsMatch() } }
    func checkIfTextsMatch() { self.textsMatch = textValue == inputString }
}
...
struct VisualDocs_WaysToReactToState_Combine: View {
    ...
    var body: some View {
        VStack {
            ...
            TextField("Write here:", text: $viewModel.inputString).applyDefaultLayout()
            Text(viewModel.inputString)
            Toggle(isOn: $viewModel.textsMatch) { Text("Matching?")}.disabled(true).padding()
        }.padding()
    }
}

struct VisualDocs_WaysToReactToState_Combine_PreviewProvider: PreviewProvider {
    static var previews: some View {
        VisualDocs_WaysToReactToState_Combine()
    }
}
